...
// .wrangler/tmp/bundle-jSz2jM/checked-fetch.js
...
// .wrangler/tmp/bundle-jSz2jM/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-jSz2jM/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var SHIFT_SEQUENCE = [9, 2, 7, 4, 6];
function reverseObfuscateCode(obfuscatedCode) {
  ...
  const substituted = [
    transposed[1],
    // char from 2nd pos moves back to 1st
    transposed[3],
    // char from 4th pos moves back to 2nd
    transposed[4],
    // char from 5th pos moves back to 3rd
    transposed[0],
    // char from 1st pos moves back to 4th
    transposed[2]
    // char from 3rd pos moves back to 5th
  ];
  const original = substituted.map((char, index2) => {
    const shift = SHIFT_SEQUENCE[index2];
...
__name(reverseObfuscateCode, "reverseObfuscateCode");
...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const { data, error } = await supabase.from("fileshare").select("id, p2p_offer, expires_at").eq("short_code", shortCode).single();
      if (error || !data) {
